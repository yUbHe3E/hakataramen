...
class GaussianDistance:
    """
    把原子对距离展开成高斯基——MEGNet 的 edge_attr
    dmin, dmax: 展开区间；step: 相邻中心间距 (σ 取 step)
    """
    def __init__(self, dmin: float, dmax: float, step: float):
...
def process_cif_file(file_path, species_to_onehot):
    ...
    for i in range(num_atoms):
        for j in range(i + 1, num_atoms):
            ...
            if dist < (r_i + r_j):
                # 无向图 → 存两条
                edge_index.append([i, j]); distances.append(dist)
                edge_index.append([j, i]); distances.append(dist)

    edge_index = torch.tensor(edge_index, dtype=torch.long).t()

    # ---------- B) 计算 edge_attr ----------
    gd = GaussianDistance(dmin=0.0, dmax=10.0, step=0.2)   # 你可调整超参
    edge_attr_np = gd.expand(np.asarray(distances))
...
class AdsorptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        adsorbate_str = row['adsorbate']

        # (1) 拿到图 (深拷贝以免共用同一对象)
        original_graph = self.cif_dict[zeolite_type]
        graph_data = copy.deepcopy(original_graph)  # 里面已含 edge_attr

        # (2) adsorbate one‑hot
        if not self.test:
            type_vec = self.ads_map.get(adsorbate_str,
                                        np.zeros(len(self.ads_map), dtype=float))
        else:
        ...
        ads_tensor = (torch.tensor(adsorption, dtype=torch.float)
                      - self.mean_adsorption) / self.std_adsorption

        # (5) 拼接 one‑hot  →  全局向量 u
        type_tensor = torch.tensor(type_vec, dtype=torch.float)
        temp_pressure = torch.cat([t_p_tensor, type_tensor], dim=0)  # shape = (2 + n_ads,)

        # (6) 写回 Data ─── ❶❷ 仅两行新内容 ──────────────
        # graph_data.u = temp_pressure  # ❶ 供 MEGNet 直接使用
        graph_data.y = ads_tensor.unsqueeze(0)  # shape (1,)
        # 若你仍有其他模型要用 temp_pressure，可保留 ↓
        graph_data.temp_pressure = temp_pressure.unsqueeze(0)
