...
def train(model, criterion, optimizer, train_loader, test_loader, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        joblib.dump(model, "./skANNall.model")
        # 计算并打印出训练集和测试集的MSE
        with torch.no_grad():
            model.eval()
            train_mse = 0.0
...
model = joblib.load("./skANNall.model")
...
print("Shape of prediction_get:", prediction_get.shape)
print("Shape of scaler_y.min_:", scaler_y.min_.shape)
print("Shape of scaler_y.scale_:", scaler_y.scale_.shape)
